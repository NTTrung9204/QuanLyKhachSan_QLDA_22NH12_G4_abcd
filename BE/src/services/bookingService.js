 ...
 * Get all bookings with check-in and check-out dates greater than current time and status is pending
 */
exports.getFuturePendingBookings = async () => {
  const currentDate = new Date();
  
  const query = {
    status: 'pending',
    'rooms.checkIn': { $gt: currentDate },
    'rooms.checkOut': { $gt: currentDate }
  };
